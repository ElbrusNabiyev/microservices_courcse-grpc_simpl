...
import (
	...
	pb "github.com/ElbrusNabiyev/microservices_courcs/grpc_simpl/pkg/api/notes"
...
type server struct {
	// UnimplementedNotesServiceServer must be embedded to have forward compatible implementation
...
func (s *server) SaveNote(_ context.Context, req *pb.SaveNoteRequest) *pb.SaveNoteResponse {
	info := req.GetInfo()
...
func (s *server) ListNotes(_ context.Context, req *pb.ListNotesRequest) *pb.ListNotesResponse {
...
func main() {
	...
	//SaveNote:
	// grpc_cli call --json_input --json_output localhost:8082 NotesServiceServer/SaveNote '
	// ListNotes:
	// grpc_cli call --json_input --json_output localhost:8082 NotesServiceServer
